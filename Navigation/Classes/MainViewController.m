...
- (void)resetMainView
{
    ...
    if (_leftPanelViewController != nil)
    {
        ...
        [self.leftPanelViewController willMoveToParentViewController:nil];
    ...
    if (_rightPanelViewController != nil)
    {
        ...
        [self.rightPanelViewController willMoveToParentViewController:nil];
        [self.rightPanelViewController removeFromParentViewController];
...
-(void)movePanel:(id)sender
{
    ...
    // Tracking the Location（手势相对于视图移动的距离） and Velocity（pixels/second）
    ...
    if([(UIPanGestureRecognizer*)sender state] == UIGestureRecognizerStateEnded)
    {
        ...
        if(velocity.x > 0)
        {
            // NSLog(@"gesture went right , %f",velocity.x);
        } else {
            // NSLog(@"gesture went left  , %f",velocity.x);
        }
    ...
    if([(UIPanGestureRecognizer*)sender state] == UIGestureRecognizerStateChanged)
    {
        ...
        // Are you more than halfway? If so, show the panel when done dragging by setting this value to YES.显示侧边栏，无论左右
